...
import { Users, BedDouble, Sparkles, MapPin } from "lucide-react";
...
export default function ChaletCard({ chalet }) {
  const firstImg = chalet.images?.[0] ? fileUrl(chalet.images[0]) : FALLBACK_IMG;
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-1 shrink-0 text-sm">
            ...
            <span className="text-xs text-inkSoft mr-1">({chalet.reviews_count || 0})</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 text-sm text-inkSoft">
          ...
          {chalet.google_maps_url && (
            <div className="flex items-center gap-1.5 text-gold">
              <MapPin size={14} strokeWidth={1.5} /> الموقع
            </div>
          )}
        </div>
        ...
        <div className="pt-4 border-t border-border/50 flex items-end justify-between">
          <div>
            <div className="text-xs text-inkSoft mb-1">
              {chalet.starting_price > 0 ? "يبدأ من" : "سعر متغير"}
            </div>
            <div className="font-heading text-2xl text-forest">
              {chalet.starting_price > 0
                ? <>{chalet.starting_price?.toLocaleString("ar")} <span className="text-sm text-gold font-body">₪</span></>
                : <span className="text-sm text-inkSoft font-body">حسب الموعد</span>}
            </div>
          </div>
